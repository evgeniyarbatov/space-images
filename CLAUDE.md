...
- Shared helpers live in `scripts/common.py`. Entry points write under `$(DATA_DIR)` (default `~/data/space-images`, see Makefile): `inspire.py` → `album/daily/`; `nasa.py` → `images/`; `planets.py` → `images/<planet>/`.
...
- Downloaded and generated content is written outside the repo tree, under `$(DATA_DIR)`; nothing generated is committed or gitignored inside the repo itself.
